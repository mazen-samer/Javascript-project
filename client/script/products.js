...
function display() {
    ...
    for (var i = 0; i < data.length; i++) {
       ...
       <div class="card m-7  w-96 h-[450px] sm:w-72 md:w-80 lg:w-80 rounded-xl shadow-lg overflow-hidden 
         transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-2xl" data-id="${data[i].id}">
            <img class="p-8 rounded-t-lg w-full h-[60%] h-64 sm:h-72 md:h-80 lg:h-50 object-contain transition-transform duration-300 ease-in-out hover:scale-110" src="${data[i].image}" alt="${data[i].title}" />
        </a>
        <div class="px-5 pb-5">
            <a href="#">
                <h5 class="text-l font-semibold text-gray-900 dark:text-white line-clamp-1">${data[i].title}</h5>
            </a>
            ...
            <div class="flex items-center justify-between">
                <span class="text-xl font-bold text-gray-900 dark:text-white">$${data[i].price}</span>
            </div>
        </div>
    </div>
</div> ` ;
